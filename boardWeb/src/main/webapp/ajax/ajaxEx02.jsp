...
<script>
				...
				if(request.status == 200){
					...
					for(var i=0; i<jObj.length; i++){
						...
						for(var j =0; j<field3.length;j++){
							document.getElementById("result").innerHTML +=
								field3[j].subField1 + "," + field3[j].subField2
						}
					}
	...
	function callXML(){
		...
		request.onreadystatechange = function(){
			if(request.readyState == 4){
				if(request.status == 200){
					var xml  = request.responseXML; //돔트리구조로 변환된 xml document를 얻어올 수 있음
					
					var books = xml.getElementsByTagName("book");
					
					//console.log(books);
					
					//책의 이름, 출판사, 저자, 가격 정보를 result 태그에 전부 출력하세요 
		...
		//xml2 버튼 클릭시  data.xml 에 있는 모든 서브아이템의 네임 태그 값을 result 태그에 출력하세요 
